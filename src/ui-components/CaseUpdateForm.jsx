...
export default function CaseUpdateForm(props) {
  const {
    id: idProp,
    case: caseModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = caseRecord
      ? { ...initialValues, ...caseRecord }
      : initialValues;
    setJurisdiction(cleanValues.jurisdiction);
    setCourt(cleanValues.court);
    setJudge(cleanValues.judge);
    setProsecutor(cleanValues.prosecutor);
    setCaseNumber(cleanValues.caseNumber);
    setCaseName(cleanValues.caseName);
    setErrors({});
  };
  const [caseRecord, setCaseRecord] = React.useState(caseModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Case, idProp)
        : caseModelProp;
      setCaseRecord(record);
    };
    queryData();
  }, [idProp, caseModelProp]);
  React.useEffect(resetStateValues, [caseRecord]);
  const validations = {
    jurisdiction: [],
    court: [],
    judge: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Case.copyOf(caseRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CaseUpdateForm")}
      {...rest}
    >
      <TextField
        label="Jurisdiction"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || caseModelProp)}
